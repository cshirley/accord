...
- Host-neutral prompt assets live in `packages/accord-assets/` (agents, providers, lang-profiles). Standalone Pi skills live in `packages/pi-skills/` (loaded via `pi.skills`, not `install:assets`). Pi-only CI templates remain under `packages/pi-accord/assets/`.
