...
| **Jira** (`jira`)                             | `mcp__atlassian__jira_get_issue`                                                  | An Atlassian/Jira-capable MCP server whose key matches the prefix you see in Pi (either `atlassian`).                                                       |
...
| **Slack** (`slack`)                           | `mcp__slack__search_messages`                                                     | A Slack MCP server registered as `slack` (or adjust [`accord-config.json`](docs/configuration.md) / project providers if your server uses a different key). |
