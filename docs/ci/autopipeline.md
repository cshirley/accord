...
`packages/accord-ci/src/lib/inputs.ts` for the source of truth.
...
| `harness` | string | no | `claude` | Default exec harness backend (`claude`, `cursor`, `pi`) written to `~/.config/accord/accord.json`. |
| `accord_ref` | string | no | `v1` | Git ref of the **accord monorepo** checked out into `.accord-ci/` (tag, branch, or SHA). |
...
secret, checkout failure, `setup-accord` install failure, runner-level
timeout.
...
The autopipeline runs phases via **`accord-cli`** with an exec harness
(Claude Code by default). Model tier selection uses `subagent.json` under
`~/.config/pi/agent/` — that file does not exist on a fresh runner.

| File | Reader | Effect when missing |
|---|---|---|
| `subagent.json` | Exec harness model resolution | Falls back to in-code defaults (works but can't be tuned per-phase). |
| `~/.config/accord/accord.json` | `accord-cli` harness backends | Written by `accord config init` in `setup-accord`. |

The `setup-accord` composite (`.github/actions/setup-accord/action.yml`)
checks out the accord monorepo, runs `bun install`, installs the Claude Code
CLI when `harness=claude`, runs `accord config init --write`, and seeds
`subagent.json` from `packages/accord-cli/ci/` using `cp -n` so a
cache-restored copy always wins.
...
fall back to in-code defaults in the exec harness.
...
seeded template in their wrapper workflow, **after** the `setup-accord`
action:
...
Phase orchestration is via **`accord-cli`** in-process orchestration
(`packages/accord-ci/src/run-phase.ts` delegates to
`runDevSubcommandOrchestrationWithReplans`, `runResumeOrchestrationWithReplans`,
and `runFinishOrchestrationFromResolution`). There is **no `pi -p --mode json /dev`
subprocess** and **no SDK import** — `packages/accord-ci/tests/no-extra-pi-spawns.test.ts`
asserts no `pi /dev` invocations under CI scripts/actions and that no file under
`packages/accord-ci/src/` imports `@earendil-works/pi-coding-agent`. ACCORD's own `dev_*` tools
...
|---|---|---|
| L1 — unit | All deterministic logic: gates, brief seed, bootstrap, resume gate, terminal dispatch, cost cap, commit/PR shape. | `bun test packages/accord-ci/tests` |
| L2 — scenarios | Ten-scenario self-test (AC-17) + exit-code policy (AC-18). | `bun test packages/accord-ci/tests/self-test packages/accord-ci/tests/exit-code.test.ts` |
...
| L4 — real runner | Real GitHub-hosted runner, `dry_run=true` so no Jira/PR side effects. | `bun run smoke:gh:autopipeline` after `git push` — uses current branch for workflow ref, `base_branch`, and `accord_ref` (see [`pre-merge-smoke.md`](pre-merge-smoke.md)) |
| L5 — wet run | End-to-end: sandbox Jira → Automation rule → real exec-harness phase runs → PR. | See `docs/ci/consumer-quickstart.md`. |
...
The L3 wrapper (`packages/accord-ci/src/smoke-act.ts`) auto-detects a Docker
...
`@clive.shirley/accord-ci` (`packages/accord-ci/`). On a consumer
runner, `setup-accord` checks out this repo into `.accord-ci/`; workflow
steps invoke scripts as
`bun run .accord-ci/packages/accord-ci/src/<script>.ts`. Local
...
| Concern | File |
|---|---|
| CI package | `packages/accord-ci/` (`package.json`, `src/`, `tests/`) |
| Workflow entrypoint | `.github/workflows/autopipeline.yml` |
| Composite — install accord-cli + cache | `.github/actions/setup-accord/action.yml` |
| Composite — invoke a phase | `.github/actions/run-accord-phase/action.yml` |
| Gate — AGENTS.md (AC-2) | `packages/accord-ci/src/gate-agents-md.ts` |
| Gate — Jira ticket (AC-3) | `packages/accord-ci/src/gate-ticket.ts` |
| Brief seeding (AC-5) | `packages/accord-ci/src/seed-brief.ts` |
| Work item bootstrap (AC-5, AC-6) | `packages/accord-ci/src/bootstrap-work-item.ts` |
| Resume gate (AC-16) | `packages/accord-ci/src/decide-resume.ts` |
| Phase runner + parsing | `packages/accord-ci/src/run-phase.ts` |
| Terminal dispatch + cost cap | `packages/accord-ci/src/parse-phase-result.ts` |
| Jira REST helper | `packages/accord-ci/src/jira-comment.ts` |
| Commit + PR open (AC-8, AC-9, AC-11) | `packages/accord-ci/src/commit-and-pr.ts` |
| Self-test harness (AC-17) | `packages/accord-ci/tests/self-test/` |
