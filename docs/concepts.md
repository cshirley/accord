...
- **Adapters** contain host-specific glue: Pi (`packages/pi-accord/src/adapters/pi/`) and stdio MCP (`packages/accord-mcp/`) — both expose the same `dev_*` tools from `accord-core`.
