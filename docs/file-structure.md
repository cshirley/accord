...
packages/                      Bun workspaces — Pi extensions bundled with pi-accord
  ...
  pi-accord/                   Pi extension (/dev, hooks, assets) — npm: `@clive.shirley/pi-accord`
    src/
      ...
      extension.ts             /dev command, autocomplete, tools, hooks, status bar
      cli-client.ts            accord-cli delegate (in-process or subprocess)
      subagent/                Spawn bridge, runtime host, judgment, spawn UI
      command/                 /dev autocomplete
...
- Change Pi-specific command behavior, autocomplete, hook listeners, status, or tool envelope wrapping in `packages/pi-accord/src/`.
...
- Change subagent prepare/preflight/result handling in `packages/accord-core/src/subagent/`; Pi spawn bridge in `packages/pi-accord/src/subagent/`.
...
Core modules live in `accord-core` and stay host-neutral; Pi-specific APIs belong in `packages/pi-accord/src/`.
