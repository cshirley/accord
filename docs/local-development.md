...
| Piece | How | Why |
|---|---|---|
| **This repo as a Pi package** | Add the repo root to global `settings.json` → **`packages`** via **`pi install <path>`** (see [One-time setup](#one-time-setup)). Pi reads `package.json` → **`pi`** and loads **`pi.extensions`** (six modules: `packages/pi-subagent`, `packages/pi-worktree`, `packages/pi-thrift`, `packages/pi-git-tools`, `packages/pi-integrations`, then `src/index.ts` for `/dev`, `dev_*` tools, hooks) plus **`pi.skills`** / **`pi.prompts`** / **`pi.themes`** from the checkout. Same manifest-driven behaviour as the old `extensions/accord` symlink, without linking into `extensions/`. |
