...
The diagrams above reflect **current** behaviour: deterministic hooks plus the **core orchestrator** (`packages/accord-core/src/orchestration/`) driving `/dev` workflow subcommands via programmatic `subagent` spawns (default on; set `ACCORD_CORE_ORCHESTRATOR=0` to disable). The Pi adapter delegates execution to `@clive.shirley/accord-cli` (in-process or subprocess) and implements host ports (`spawnSubagent`, optional `runJudgment`) in `packages/pi-accord/src/adapters/pi/subagent/`. The same loop runs headlessly via `bun run accord resume <ID>`. See [`harness-orchestration.md`](harness-orchestration.md), [`accord-cli.md`](accord-cli.md). **Stdio MCP** consumers use **`dev_orchestrate`** (plan JSON; optional execution when `ACCORD_MCP_HARNESS` is set) — see [`hooks-and-tools.md`](hooks-and-tools.md).
