/** Exec harness subprocess template (see `schemas/accord-schema.json`). */
export interface ExecHarnessConfig {
  /** argv template; tokens: {{agentId}}, {{agent}}, {{task}}, {{taskFile}}, {{cwd}} */
  command: string[];
  /** Where to parse the return JSON packet from (default: stdout). */
  response_json?: "stdout" | "stderr";
  /** Extra env vars merged over process.env for each spawn. */
