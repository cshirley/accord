...
/** Host-specific fields merged onto {@link DevOrchestratePayload} for clients. */
export interface DevOrchestrateEnrichedPayload extends DevOrchestratePayload {
  /** Harness backend when spawns are available (`pi`, `exec`, `cli`, …). */
  harness?: string;
  /** Populated when the host executed the orchestration loop. */
  execution?: DevOrchestrateExecutionResult;
}

export interface DevOrchestrateHostHints {
  harness?: string;
  programmatic_spawn_supported: boolean;
  /** When true and {@link ToolHandlerContext.executeOrchestration} is set, run resume/finish. */
  execute_by_default?: boolean;
}
...
export function enrichDevOrchestratePayload(
  payload: DevOrchestratePayload,
  hints: DevOrchestrateHostHints,
  execution?: DevOrchestrateExecutionResult,
): DevOrchestrateEnrichedPayload {
  return {
    ...payload,
    programmatic_spawn_supported: hints.programmatic_spawn_supported,
    ...(hints.harness ? { harness: hints.harness } : {}),
    ...(execution ? { execution } : {}),
  };
}
