...
import { CORE_DIR } from "../config/paths.js";
...
/** Minimal session reader for orchestrator usage attribution (Pi ExtensionContext-compatible). */
...
function resolvePricingPath(): string | null {
  try {
    const candidate = path.join(CORE_DIR, "schemas", "model-pricing.json");
...
function extractTextFromUserMessageContent(content: unknown): string {
  if (!content) return "";
...
function collectUserTextsFromBranch(ctx: OrchestratorSessionContext): string[] {
  const out: string[] = [];
  try {
