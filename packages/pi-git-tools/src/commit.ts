...
function inferTicketSuggestions(
	...
	const allPaths = (pattern: RegExp) =>
		paths.length > 0 && paths.every((p) => pattern.test(p));
	...
	if (/\b(add|adds|support|enable|introduce|infer|suggest|new)\b/.test(haystack) || (codeChanged && hasAddedLines)) {
		add("FEATURE", 80, "Source changes add or expand behavior");
	}
	if (hasPath(/(^|\/)(settings|config|configs|\.config)\b|(^|\/)(package|tsconfig|AGENTS|Brewfile)|\.(json|toml|ya?ml)$/)) {
	...
	return [...suggestions.values()].sort(
		(a, b) =>
			b.score - a.score ||
			COMMIT_TYPE_PREFIXES.indexOf(a.value) -
				COMMIT_TYPE_PREFIXES.indexOf(b.value),
	);
}
...
export function registerCommitTools(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		promptSnippet:
			"Gather git context (status/diff/log/branch/secrets/artifacts) in one call",
		...
		async execute(_toolCallId, _params, signal, onUpdate, ctx) {
			...
			const artifacts =
				artifactPaths.length > 0
					? await readArtifacts(artifactPaths, cwd)
					: [];

			const artifactTicket = !ticket
				? (artifacts.find((a) => a.ticket)?.ticket ?? null)
				: null;
			const resolvedTicket = ticket ?? artifactTicket;
			const ticketSuggestions = resolvedTicket || safeFiles.length === 0
				? []
				: inferTicketSuggestions(
						safeFiles,
						diffStat.trim(),
						smartDiff.diff,
						branch,
	...
	pi.registerTool({
		...
		description:
			"Stage files individually and commit. Returns hash and status.",
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			await withTempFile(message, (msgFile) =>
				git(["commit", "-F", msgFile], cwd, signal),
			);

			const hash = (
				await git(["rev-parse", "--short", "HEAD"], cwd, signal)
			).trim();
