...
export function registerPrTools(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		promptSnippet:
			"Gather PR context (existing PR/commits/diffstat/spec/verify) in one call",
		...
		async execute(_toolCallId, _params, signal, onUpdate, ctx) {
			...
			// Get branch + ticket
			const branch = (
				await git(["branch", "--show-current"], cwd, signal)
			).trim();
			const ticket = extractTicket(branch);

			// Run remaining commands in parallel
			const [existingPrRaw, commitsRaw, diffStatRaw, specFiles] =
				await Promise.all([
					ghAuth
						? gh(
								[
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			title: Type.Optional(
				Type.String({
					description:
						"PR title (creates new PR if provided). Format: [TICKET-ID] summary",
				}),
			),
			body: Type.Optional(
				Type.String({ description: "PR body markdown" }),
			),
		}),
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			try {
				...
				if (
					...
					throw new Error(
						`Push rejected (non-fast-forward). Pull or rebase first.\n${msg}`,
					);
				}
				throw err;
			}
			...
			const createOutput = await withTempFile(
				body ?? "",
				(bodyFile) =>
					gh(
						[
