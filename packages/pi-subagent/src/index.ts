...
/**
 * Combine a resolved model id with its provider.
 *
 *   - Empty/undefined model: return undefined (let pi pick session default).
 ...
 */
function qualifyModel(model: string | undefined, provider: string): string | undefined {
...
async function runSingleAgent(
  ...
  // Resolve agent → concrete provider, model, and thinking recipe. The
  // resolver walks frontmatter overrides → skill profile → activeProfile →
  // defaultProfile, then qualifies the model with the resolved profile's
  // provider. Thinking is then expressed in whatever way the profile declares
  // (separate flag, embedded in the model id, OpenAI reasoning-effort, or
  // disabled) so we don't blindly push `--thinking` against providers that
  // don't accept it.
  const resolved = resolveModelConfig(agent);
  const qualifiedModel = resolved ? qualifyModel(resolved.model, resolved.provider) : undefined;
  if (qualifiedModel) args.push("--model", qualifiedModel);
  if (resolved) {
    switch (resolved.thinkingMode) {
      case "flag":
        if (resolved.thinking) args.push("--thinking", resolved.thinking);
        break;
      case "reasoning_effort":
        if (resolved.reasoningEffort) args.push("--reasoning-effort", resolved.reasoningEffort);
        break;
      case "embedded":
      case "none":
        break;
