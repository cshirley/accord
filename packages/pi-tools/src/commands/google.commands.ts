...
import { getGoogleAuth, setGoogleAuth, loadAuth, saveAuth, refreshGoogleToken } from "../auth.js";
import { makeGoogleRequest, hasNativeGoogleAuth } from "../services/google.client.js";
  ...
  setup: {
    ...
    handler: async ({ ui }) => {
      if (getMcpRegistry().has("google-workspace")) {
        ui.notify("✅ Google Workspace MCP fallback available.\nNative OAuth gives faster, direct access.", "info");
      }

      const hasOAuth = await ui.confirm("OAuth Access?", "Can you create OAuth2 apps in Google Cloud Console?");

      if (!hasOAuth) {
        const auth = loadAuth();
        auth.google = { handoffMode: true, accessToken: "", refreshToken: "", expiresAt: 0, clientId: "", clientSecret: "" } as any;
        saveAuth(auth);
      ...
      if (!clientId?.trim()) { ui.notify("Cancelled", "info"); return; }
      const clientSecret = await ui.input("Google OAuth2 Client Secret:", "");
      if (!clientSecret?.trim()) { ui.notify("Cancelled", "info"); return; }
      ...
      if (!authCode?.trim()) { ui.notify("Cancelled", "info"); return; }
      ...
      try {
        const tokenResp = await fetch("https://oauth2.googleapis.com/token", {
          ...
          body: new URLSearchParams({
            client_id: clientId, client_secret: clientSecret, code: authCode.trim(),
            grant_type: "authorization_code", redirect_uri: redirectUri,
          }),
        });
        if (!tokenResp.ok) throw new Error(`Token exchange failed: ${tokenResp.status}`);
        const td = await tokenResp.json() as { access_token: string; refresh_token: string; expires_in: number };
        setGoogleAuth(td.access_token, td.refresh_token, td.expires_in, clientId, clientSecret);
        const profile = await makeGoogleRequest("https://www.googleapis.com/oauth2/v2/userinfo") as { name: string; email: string };
        ui.notify(`Google configured! ✅  User: ${profile.name} (${profile.email})`, "info");
      } catch (error) {
        ui.notify(`Setup failed: ${error instanceof Error ? error.message : String(error)}`, "error");
      }
    },
  },
    ...
    test: async () => {
      if (hasNativeGoogleAuth()) {
        try {
          ...
          if (token) {
            const profile = await makeGoogleRequest("https://www.googleapis.com/oauth2/v2/userinfo") as { name: string; email: string };
            return { ok: true, message: `Google connected (OAuth): ${profile.name} (${profile.email})` };
          }
        } catch { /* fall through */ }
      }
