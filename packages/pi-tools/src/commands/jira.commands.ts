  ...
  setup: {
    ...
    handler: async ({ ui }) => {
      ...
      if (!email?.trim()) {
        ...
        if (!email) { ui.notify("Cancelled — no email and $JIRA_EMAIL not set", "info"); return; }
      ...
      if (!apiToken?.trim()) {
        ...
        if (!apiToken) { ui.notify("Cancelled — no token and $JIRA_API_KEY not set", "info"); return; }
      ...
      if (!baseUrl?.trim()) { ui.notify("Cancelled", "info"); return; }
      ...
      try {
        ...
        ui.notify(`Jira setup failed: ${error instanceof Error ? error.message : String(error)}`, "error");
      }
    },
  },
  ...
  status: {
    ...
    test: async () => {
      ...
      try {
        ...
        return { ok: true, message: `Jira connected: ${user.displayName} (${auth.email}) at ${auth.baseUrl}` };
      } catch (error) {
        return { ok: false, message: `Jira connection failed: ${error instanceof Error ? error.message : String(error)}` };
      }
    },
  },
});
