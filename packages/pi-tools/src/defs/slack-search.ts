...
export default defineTool<
  ...
  { total: number; messages: Array<{ text: string; user: string; channel: string; permalink: string }> }
>({
  name: "slack-search",
  label: "Slack Search",
  ...
  params: {
    query:     { type: "string", required: true, description: "Search query (supports Slack search syntax)" },
  ...
  async execute(p) {
    const resp: SlackSearchResponse = await makeSlackRequest("search.messages", {
      query: p.query, sort: p.sort || "timestamp",
      sort_dir: p.sort_dir || "desc", count: p.count || 20,
      highlight: p.highlight !== false,
    });

    const messages = resp.messages.matches.map((m) => ({
      text: m.text, user: m.user,
  ...
  format(result) {
    const lines = result.messages.map((m) =>
      `  • [${m.channel}] ${m.user}: "${m.text.slice(0, 120)}" — ${m.permalink}`,
    ).join("\n");
    return {
