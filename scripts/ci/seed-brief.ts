...
function renderBrief(ticket: JiraIssue, generatedAt: string): string {
  ...
  lines.push("");
  lines.push(`*Seeded from Jira by the ACCORD autopipeline. Ticket ${ticket.key}, status ${ticket.fields.status.name}, type ${ticket.fields.issuetype.name}.*`);
  lines.push("");

  lines.push("## Core Problem");
  lines.push("");
  lines.push("Synthesised verbatim from the Jira ticket description. The autopipeline trusts the ticket-gate (AC-3) to have already verified the description carries problem framing, ACs, scope, and target paths.");
  lines.push("");
  ...
  lines.push("");
  lines.push(`Implement the acceptance criteria listed in ${ticket.key} ("${ticket.fields.summary}") and land them as a single reviewable PR. Verification is the project-level test/typecheck/lint set declared in AGENTS.md → \`## Dev Harness\`.`);
  lines.push("");

  lines.push("## Scope");
  lines.push("");
  lines.push(`- **Target paths:** as declared in the ticket's \`## Target paths\` section (or \`## Areas\` / \`## Target area\`).`);
  lines.push("- **Out of scope:** as declared in the ticket's `## Out of scope` section. The autopipeline will reject deviations beyond that set.");
  ...
  lines.push("");
  lines.push("Phase agents may treat the gathered-context block as authoritative for the Jira facts above and SHOULD skip `phase-gather` for this work item.");
  lines.push("");
  return lines.join("\n");
}
