...
const DEVIATION_ACTIONS: AutocompleteItem[] = [
  { value: "accept", label: "accept", description: "Accept a deviation and record plan guidance" },
  {
    value: "revert",
    label: "revert",
    description: "Reject a deviation and return to plan-conformant work",
  },
];
...
function getDeviationActionCompletions(
  workItemId: string,
  query: string,
): AutocompleteItem[] | null {
  const q = query.toLowerCase();
  const items = DEVIATION_ACTIONS.filter((action) => action.value.startsWith(q)).map((action) => ({
    ...action,
...
function getDeviationTaskCompletions(
  ...
  const deviations = (wi.deviations || []).filter((deviation) => {
    const haystack =
      `task-${deviation.task_id} ${deviation.task_id} ${deviation.description || ""}`.toLowerCase();
    return haystack.includes(q);
  });
...
export function getDevArgumentCompletions(prefix: string): AutocompleteItem[] | null {
  ...
  if (subcommand === "deviations") {
    ...
    if (words.length === 2 && hasTrailingSpace)
      return getDeviationActionCompletions(workItemId, "");
    if (words.length === 3 && !hasTrailingSpace)
      return getDeviationActionCompletions(workItemId, action);
    if (
      words.length === 3 &&
      hasTrailingSpace &&
      DEVIATION_ACTIONS.some((a) => a.value === action)
    ) {
  ...
  if (WORK_ITEM_ID_SUBCOMMANDS.has(subcommand)) {
    ...
    if (words.length === 2 && !hasTrailingSpace)
      return getWorkItemCompletions(subcommand, words[1] ?? "");
  }
  ...
  const filtered = DEV_SUBCOMMANDS.map((s) => ({
