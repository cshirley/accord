...
function titleCaseStatus(value: unknown): string {
  return oneLine(value).replace(/_/g, " ").replace(/\b\w/g, (m) => m.toUpperCase());
}
...
function renderMarkdownReport(id: string, wi: any, report: any, verifyPath: string, summary: {
  pass: number;
  fail: number;
  partial: number;
  notVerified: number;
  ...
  for (const criterion of (report.criteria || [])) {
    ...
    if (status !== "pass") {
      ...
      lines.push(`Suggested action: ${oneLine(criterion.suggested_action || "No suggested action recorded.")}`);
      lines.push("");
    }
  }
  ...
  lines.push(report.verdict === "pass" ? "Next: `/commit` then `/pr`." : `Next: \`/dev gaps ${id}\`.`);
  return `${lines.join("\n").replace(/\n{3,}/g, "\n\n")}\n`;
}
...
export function devVerifySummary(id: string): VerifySummary | { error: string } {
  ...
  let pass = 0, fail = 0, partial = 0, notVerified = 0;
  ...
  for (const c of (report.criteria || [])) {
    ...
    switch (status) {
      ...
      case "not_verified": notVerified++; break;
    }
    if (status !== "pass" && (c.gap || c.suggested_action)) {
      gaps.push({ ac_id: c.ac_id || c.id, gap: c.gap || "", suggested_action: c.suggested_action || "" });
    }
  }

  const lines: string[] = [`Verdict: ${report.verdict}`, `Verify: ${verifyPath}`, `  pass=${pass}  fail=${fail}  partial=${partial}  not_verified=${notVerified}`];
  if (gaps.length > 0) {
    lines.push("\nGaps:");
    for (const g of gaps) {
  ...
  fs.writeFileSync(markdownPath, renderMarkdownReport(id, wi, report, verifyPath, {
    pass,
    fail,
    partial,
    notVerified,
  ...
  return { verdict: report.verdict, verify_path: verifyPath, markdown_path: markdownPath, pass, fail, partial, not_verified: notVerified, gaps, formatted: lines.join("\n") };
}
